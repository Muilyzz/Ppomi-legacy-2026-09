...
enum Timeline {
    ...
    @MainActor static func receive(_ message: Any, state: AppState) {
        ...
        if let move = message["moveLens"] as? [String: Any], let name = move["lens"] as? String, let x = move["x"] as? Int, let y = move["y"] as? Int {
            try? LensStore.move(lens: name, x: x, y: y, kind: move["kind"] as? String, dbPath: AppSettings.dbPath)
            state.reloadLedger()
        }
        if let name = message["newLens"] as? String {
            try? LensStore.add(name: name, kind: message["kind"] as? String, dbPath: AppSettings.dbPath)
            state.reloadLedger()
        }
    }
}
