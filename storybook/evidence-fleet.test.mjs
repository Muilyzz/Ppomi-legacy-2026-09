...
test('큰 면: 서버 숫자, presence, 네 링크 상태, 다단. id·파일 바이트 없음', () => {
  ...
  assert.match(h, /Mac · 세금계산서/);
  assert.match(h, /Win · 카드/);
  assert.match(h, /Phone · 스냅샷\(보조\)/);
...
test('열기는 onOpen, 오프라인 버튼은 호출 안 함', () => {
  ...
  assert.match(el.innerHTML, /disabled>Phone · 스냅샷\(보조\)</);
...
test('스냅샷 행은 적격으로 안 보이고, 빈 기기·적대 문자열은 이스케이프', () => {
  ...
  assert.match(snap, /Phone · 스냅샷\(보조\)/);
