...
export class CredentialManagerSecretStore implements SecretStore {
  ...
  put(key: string, value: string): void {
    assertKey(key);
    assertValue(value);
    const result = this.#run("put", key, value);
  ...
  #run(op: "put" | "get" | "delete", key: string, value?: string) {
    const extraEnv: Record<string, string> = { PPOMI_SECRET_OP: op, PPOMI_SECRET_KEY: key };
