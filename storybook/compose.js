// storybook/compose.js — 목록 + 절차 조합. 위는 값 종류 뷰(표·트리맵), 행을 고르면 그 기록의 플레이북(ref 필드 pkg)을 아래에 끼운다. 조합은 페이지 몫이고 두 뷰는 서로 모른다.
...
export function listWithPlaybook(list, initial, resolve) {
  ...
  X.mount(top, {...list, selected: initial ? initial.id : null, onSelect: show});
  show(initial);
  return el;
}
