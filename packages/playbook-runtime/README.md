...
| `adapter-android` | one OS | Android click / type / read-screen / focus over existing `android_*` tools. Fixture-only this slice. |
...
A later `adapter-windows` package should wrap the existing `executors/windows` tools (`screen_read`, `ui_tap`, `ui_type`, `app_open`). `adapter-macos` maps the same OS port onto Mac native names. `adapter-android` maps the same OS port onto existing `android_open` / `android_screen` / `android_click` / `android_type`. `adapter-playwright` implements `BrowserPageAdapter` and exposes those page operations as local Vercel AI SDK tools (`createPlaywrightPageAiTools`). Native / cert UI stays on `OsAdapter`.
...
| Native windows, system dialogs, cert UI, non-DOM chrome | `PlaybookRuntime` | `OsAdapter` | `adapter-windows` (UIA), `adapter-macos` (AX), `adapter-android` (accessibility) |
...
Ppomi onboarding and playbook packages have **no device-approval or Mac-approver gate**. Do not add approved-device checks to `playbook-runtime`, `adapter-windows`, `adapter-macos`, `adapter-android`, `adapter-playwright`, `playbook-kr-cert`, or their tests.

`StepResult.adapter` includes `os-android` as the Android sibling of `os-windows` / `os-macos`. `phone` stays the iPhone-mirroring adapter.
