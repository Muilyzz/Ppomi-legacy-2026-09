...
텍스트 대화는 앱과 같은 에이전트 서버([config.js](web/config.js)의 `AGENT_ENDPOINT`)로 간다. 브라우저는 Supabase 세션 토큰과 `X-Ppomi-Device`(이 브라우저의 등록 기기)를 붙여 `/v1/session`·`/v1/responses`만 부르고, 모델·Gateway 키는 그 서버에만 있다. 대화 원문은 클라이언트가 기록 키로 암호화한 뒤 `ppomi_transcript_*` RPC로 올리며, Realtime으로 암호문 INSERT를 받아 이 브라우저에서 푼다. 서버 쪽 Origin 허용 목록은 [agent/server/README.md](../agent/server/README.md)를 따른다. 통화·기기 제어·기억 저장은 웹에 없고 화면과 지침이 그렇게 말한다. 번들을 다시 만들면 [service-worker.js](service-worker.js)의 캐시 버전을 올린다.
...
| 대화 transcript E2E·Realtime | [transcript-crypto.js](web/transcript-crypto.js), [transcript-client.js](web/transcript-client.js), [transcript-realtime.js](web/transcript-realtime.js), [transcript-session.js](web/transcript-session.js) | [대화 암호](test/web-transcripts-crypto.test.js), [세션·Realtime](test/web-transcript-session.test.js) |
