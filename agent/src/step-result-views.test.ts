...
import { parseStepResultsJson } from "../../packages/ppomi-body/src/step-result";
import {
  containedImageRect,
  evidencePhase,
  evidenceScreenshot,
...
test("fixture JSON is a StepResult run covering every status", () => {
  ...
  assert.deepEqual(new Set(parsed.map(step => step.driver)), new Set(["page", "os-windows", "os-macos", "os-android", "phone"]));
  assert.doesNotMatch(fixtureJson, /"adapter":/, "rows carry driver, never the deprecated adapter alias");
  assert.ok(parsed.filter(step => step.code).length >= 4, "structured codes ride along where the core would emit them");
...
test("StepTimeline renders status colors and calls onSelect", () => {
  ...
  assert.match(html, /#next/);
  assert.match(html, /<code class="step-card-code">screen_missing<\/code>/, "code is rendered where present");
  assert.match(html, /<code class="step-card-code">commit<\/code>/);
  assert.equal((html.match(/step-card-code/g) ?? []).length, fixtureSteps.filter(step => step.code).length);
  assert.match(html, /Windows · 시간 초과/);
  assert.match(html, /서명 \(button\)/, "accessibility role rides along with the name");
  assert.doesNotMatch(html, /os-windows|os-macos|os-android/, "driver ids are labelled, not printed");
  assert.equal(driverLabel("os-macos"), "Mac");
  assert.equal(driverLabel("os-android"), "Android");
  assert.equal(driverLabel("page"), "웹 페이지");
  assert.equal(statusLabel("needs_human"), "사람 차례");
