...
구글 계정 기기(Mac·iPad·Windows·웹)는 `ppomi_register_device`로 등록되면 **바로 작업 공간 구성원**이다(`20260911120000_membership_auto_approve.sql`, MZZ-27). Google 로그인 + 구성원이면 대화·에이전트 세션·공유 작업·문서를 쓸 수 있고, Mac 기기 승인을 기다리지 않는다. `ppomi_device_revoke`는 기기를 끊고 감싼 장부 키 사본을 지운다. 해지된 기기가 다시 등록하면 다시 구성원이며 옛 감싼 사본은 지운다. 장부 기록 키(ppomi-wrap-v1)만 별 경로다: 키를 가진 기존 기기(보통 Mac)가 켜져 있으면 `ppomi_devices_waiting`으로 자동 감싼다. 검증은 [기기 등록 회귀 검사](../supabase/tests/device_approval_regression.sql)다.
