...
test("smoke: playbook-runtime drives android_* tools through adapter-android", () => {
...
test("missing click target does not call android_click", () => {
  const tools = new FixtureAndroidNativeTools(window);
  const adapter = new AndroidAdapter(tools);
  adapter.readScreen();
...
test("android_open refuses a package outside the allowlist", () => {
  ...
  const adapter = new AndroidAdapter(tools);
  adapter.readScreen();
  assert.throws(() => adapter.focus("com.android.vending"), error =>
...
test("type refuses a password field and does not call android_type", () => {
  ...
  const adapter = new AndroidAdapter(tools);
  adapter.readScreen();
  assert.throws(() => adapter.type("Password", "secret"), error =>
