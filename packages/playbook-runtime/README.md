...
- `RunResult.stepResults` is one `StepResult` per declared step (playbook order). Consumers read that list — a timeline iterates it, an LLM can use only `status`, and overlay uses optional `evidence` screenshot paths when present. `dumpStepResults(result.stepResults)` writes the run. `RunResult.evidence` remains the runner log (`StepEvidence`) of evaluated steps only. Later unreached steps are `attempt: "not_executed"` on `stepResults` only. The workbench **기록** pane (`agent/src/ui/step-record-pane.tsx`) consumes a fixture runtime run of this list. No OCR/CU or `playbook-kr-cert` in this package.
