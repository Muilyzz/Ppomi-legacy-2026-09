...
final class MCPServer {
    ...
    // ---------------------------------------------------------------- tools
    private static let reused = ["phone_screen", "phone_tap", "phone_type", "phone_key", "phone_scroll", "phone_open", "phone_installed", "phone_wait", "run_combo", "path_cold_start",
    ...
    var instructions: String {
        (Playbooks.all().first { $0.app == "공통" }?.text ?? "") +
            ...
            "\nKB 사업자 계좌·KB스타기업뱅킹을 열라는 말은 Home → KB 버튼을 누르라고 하지 말고 바로 path_cold_start(app: kb-enterprise)를 호출한다. Home 후 열고 Face ID·로그인에서 멈춘다. 계좌·비밀번호를 읽거나 채팅·로그에 쓰지 않는다. 버튼은 스모크일 뿐이다. packages/*/example 이 아니다." +
