...
One package for playbook steps, permissions, stop, and results. Recipes (the **ppomi-path**: `ppomi-path` + its JSON catalog) and surfaces (`ppomi-body-windows`, `ppomi-body-macos`, `ppomi-body-android`, `ppomi-body-iphone-mirroring`, `ppomi-body-playwright`) stay in other packages. A surface package is an eye (observation) plus a hand (actuation). `playbook-runtime`, `driver-*` and `adapter-*` are legacy package names only; this package is `ppomi-body` (`playbook-runtime` is a deprecated re-export).

Package titles are domain-specific. Do not add `core`, `common`, `engine`, `util`, `shared`, `runtime`, `adapter`, `playbook-core`, or `playbook-content`. Locked names: [docs/glossary.md](docs/glossary.md).
...
| --- | --- | --- | --- |
| `OsSurface` | `OsUiDriver` (`readScreen` / `focus` / `click` / `type`, optional `key`, `kind`) | screen texts, accessible-name `target` | `ppomi-body-windows` (UIA), `ppomi-body-macos` (AX), `ppomi-body-android`, `ppomi-body-iphone-mirroring`; fixtures via `DummyAdapter` |
...
```ts
import { DummyAdapter, FixedPermissionGate, OsSurface, Runtime } from "ppomi-body";
...
Ppomi onboarding and playbook packages have **no device-approval or Mac-approver gate**. Do not add approved-device checks to `ppomi-body`, the `ppomi-body-*` surfaces, content packs, or their tests. Permissions here are UI step permissions, not device approval.

## Contract

- **Permissions are additive.** Every step needs `ui.read` (it observes the surface); `focus`, `click`, `type`, `key`, `goto`, `fill` also need `ui.control`. A step's `require.permission` can add a requirement, never replace one. Denial stops the run before any driver call as `failed` with `code: "permission_denied"`.
- **Mutations declare an effect.** `effect: "navigate" | "input" | "commit"` on `click`, `type`, `key`, `fill` and `goto` (`focus` only foregrounds an already-running app by its accessible name and has an implied `navigate`; it is never `browser_open({ url })` or an app launch). A `key` target is the gesture name (iPhone Mirroring `phone_key home` → SpringBoard), not a screen label. A mutation with no declared effect is a `commit`, and a `commit` is never run: the run stops with `handoff` (`StepResult.status: "needs_human"`). Nothing auto-commits; payment, submit and signing buttons are always the person's step.
...
All siblings compile the core through relative imports, so they also need the `extends` line above to typecheck it (node types instead of the DOM lib). The base config's `typeRoots` points at `ppomi-body/node_modules/@types`, so run `npm ci` in `packages/ppomi-body` before `tsc` in any package.

## Tests

```sh
npm --prefix packages/ppomi-body test
```
