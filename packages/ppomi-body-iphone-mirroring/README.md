...
KB path `kb-star-biz-iphone@0.1.0`: **cold start** (first tool-call / explicit restart) sends `phone_key home` (`phone key home` = ⌘1 → SpringBoard) so AX is a known baseline, then checks that KB스타기업뱅킹 is on screen (`focus` never launches an app: opening it, Face ID / login and the account detail stay human). **Pause/resume** (continue-after-failure) passes `Runtime.run(playbook, { fromStep })` and does **not** Home — the phone is still on the failed screen; resuming past a human step needs `resumedAfterHuman: true` and is recorded in `RunResult.resumedFrom`. No payment. Grant is `ui.read` plus `ui.control` for Home, open, and the navigate click.
