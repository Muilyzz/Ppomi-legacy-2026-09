...
enum LensStore {
    /// 늘 있는 가계부(개인) 바닥. 이름이 같은 사용자 그룹은 읽을 때 지운다(그 계좌들은 가계부 미분류로).
    static let home = Lens.home
    ...
    static func add(name: String, dbPath: String) throws {
        ...
        file.lenses.append(Lens(name: name, inside: []))
    ...
    /// 그룹 상자를 판 위 다른 자리로.
    static func move(lens name: String, x: Int, y: Int, dbPath: String) throws {
        var file = read(dbPath: dbPath)
        guard let i = file.lenses.firstIndex(where: { $0.name == name }) else { return }
        file.lenses[i].x = max(0, x); file.lenses[i].y = max(0, y)
        try save(file, dbPath: dbPath)
    }
}
