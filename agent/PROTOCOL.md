...
Methods:
- `bootstrap {}` -> `{platform:"macos"|"android",deviceLabel:string,configured:boolean,endpoint:string,tools:string[]}`. Android additionally returns live `accessibility:boolean` and `controlApps:[{label,packageName}]`. Refresh before starting a session; server configuration is independent from accessibility and app authorization.
